...
class Graph:
    def __init__(self, nodes, edges, parameters, mean) -> None:
        ...
        for a, b in edges:
            self.edge_weights[(a, b)] = randomPoissonNumber(lower_edge, upper_edge, mean)
            self.edge_weights[(b, a)] = self.edge_weights[(a, b)]
            self.delay[(a, b)] = randomPoissonNumber(parameters.lower_delay, parameters.upper_delay, mean)
            self.delay[(b, a)] = self.delay[(a, b)]
        for i in range(self.nodes):
            self.node_weights[i] = randomPoissonNumber(lower_node, upper_node, mean)
            l = list()
            l.append(randomPoissonNumber(parameters.lower_x_pos, parameters.upper_x_pos, mean))
            l.append(randomPoissonNumber(parameters.lower_y_pos, parameters.upper_y_pos, mean))
