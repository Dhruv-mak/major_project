...
def node_link_mapping(sn, vneRequests, original_substrate, original_vne_list):
  ...
  while count != len(sorted_vne_req):
    ...
    vne_req = _vnode[0]
    vne_degree=_vnode[6]
    total_vnr_nodes += len(vne_req)
    for n_d in range(len(vne_req)):
      total_vnr_links += len(vne_req[n_d])
  ...
  output_dict = {
        ...
        "accepted" : len(mapped_request),
        "total_request": len(mapped_request)+len(unmapped_request),
        "pre_resource": pre_node_cost + pre_edge_cost,
        "post_resource": post_node_cost + post_edge_cost,
        "avg_bw": (edge_cost/pre_edge_cost)*100,
        "avg_crb": (node_cost/pre_node_cost)*100,
        "avg_link": (utilized_links/len(sn_link_bw))*100,
        "avg_node": (utilized_node/len(sn_crb))*100,
        "avg_path": average_path_length,
        "avg_exec": (duration),
        "total_nodes": total_vnr_nodes,
        "total_links": total_vnr_links,
    }
...
def main():
  print(f"\t\t{datetime.now().time()}\tEAA Started")
  
  # sn, vne_obj = helper.read_pickle()
  sn, vne_obj = copy.deepcopy(config.substrate), copy.deepcopy(config.vne_list)
  ...
  output_dict = node_link_mapping(substrate, vne_list, original_substrate, original_vne_list)
  print(f"\t\t{datetime.now().time()}\tEAA completed\n")
  return output_dict
