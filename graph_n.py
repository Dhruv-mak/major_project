import random
import copy
from normalDistribution import normalNumber
...
class Graph:
    def __init__(self, nodes, edges, parameters) -> None:
        ...
        for a, b in edges:
            self.edge_weights[(a, b)] = normalNumber(lower_edge, upper_edge)
            self.edge_weights[(b, a)] = self.edge_weights[(a, b)]
            self.delay[(a, b)] = normalNumber(parameters.lower_delay, parameters.upper_delay)
            self.delay[(b, a)] = self.delay[(a, b)]
        for i in range(self.nodes):
            self.node_weights[i] = normalNumber(lower_node, upper_node)
            l = list()
            l.append(normalNumber(parameters.lower_x_pos, parameters.upper_x_pos))
            l.append(normalNumber(parameters.lower_y_pos, parameters.upper_y_pos))
